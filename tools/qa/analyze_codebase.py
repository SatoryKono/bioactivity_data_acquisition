...
produces a collection of reproducible reports. The generated data now lives
under ``var/`` so that runtime artifacts stay outside the tracked source tree.
See ``docs/architecture/runtime-assets.md`` for the full directory layout:

* ``var/artifacts/module_map.json`` – mapping of python modules to their files
  and import dependencies.
* ``var/artifacts/import_graph.mmd`` – a Mermaid graph visualising
  intra-project imports.
* ``var/reports/token_clones.csv`` – clone candidates detected via a token
  normalisation strategy.
* ``var/reports/ast_clones.csv`` – clone candidates identified by structural
  AST equivalence.
* ``var/reports/semantic_clones.csv`` – clone candidates detected after a
  light semantic normalisation pass.
* ``var/reports/config_duplicates.csv`` – duplicated configuration payloads
  across ``configs/``.
