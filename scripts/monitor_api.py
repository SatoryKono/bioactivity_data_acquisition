...
def monitor_api(api_name: str, interval: int = 30, duration: int = None):
    ...
    if api_name not in apis:
        logger.info(f"Неизвестный API: {api_name}")
        logger.info(f"Доступные API: {', '.join(apis.keys())}")
        return
    
    config = apis[api_name]
    monitor = APIMonitor(api_name, **config)
    
    logger.info(f"Мониторинг {api_name.upper()} API каждые {interval} секунд")
    if duration:
        logger.info(f"Продолжительность: {duration} секунд")
    ...
    try:
        while True:
            ...
            if status["available"]:
                logger.info(f"[{timestamp}] OK {api_name.upper()} - OK ({status['response_time']}с)")
            elif status["rate_limited"]:
                logger.warning(f"[{timestamp}] WARNING {api_name.upper()} - RATE LIMITED")
            elif status["error"]:
                logger.error(f"[{timestamp}] ERROR {api_name.upper()} - ERROR: {status['error']}")
            else:
                logger.error(f"[{timestamp}] ERROR {api_name.upper()} - ERROR {status['status_code']}")
            
            # Показываем статистику каждые 10 проверок
            if check_count % 10 == 0:
                stats = monitor.get_stats()
                logger.info("\nСтатистика за последние 10 проверок:")
                logger.info(f"  Доступность: {stats['availability']}%")
                logger.info(f"  Среднее время ответа: {stats['avg_response_time']}с")
    ...
    stats = monitor.get_stats()
    logger.info("\nФинальная статистика:")
    logger.info(f"  Всего проверок: {stats['checks']}")
    logger.info(f"  Доступность: {stats['availability']}%")
    logger.info(f"  Среднее время ответа: {stats['avg_response_time']}с")
    logger.error(f"  Ошибки: {stats['errors']}")
    ...
    filename = reports_dir / f"monitor_{api_name}_{timestamp}.json"
