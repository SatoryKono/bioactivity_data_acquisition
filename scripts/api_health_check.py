...
def display_health_summary(results: list[dict]):
    ...
    logger.info("="*60)
    logger.info("СВОДКА ПО ЗДОРОВЬЮ API")
    logger.info("="*60)
    
    healthy_count = sum(1 for r in results if r["health_status"] == "healthy")
    total_count = len(results)
    
    logger.info(f"Всего API: {total_count}")
    logger.info(f"Здоровые: {healthy_count}")
    logger.info(f"Проблемы: {total_count - healthy_count}")
    logger.info(f"Общий статус: {'ЗДОРОВ' if healthy_count == total_count else 'ЕСТЬ ПРОБЛЕМЫ'}")
    ...
    for result in results:
        ...
        logger.info(f"{result['api']:15} | {status_icon:12} | {status_code:3} | {response_time:>8}")
...
def main():
    ...
    logger.info("Комплексная проверка здоровья API")
    logger.info("=" * 40)
