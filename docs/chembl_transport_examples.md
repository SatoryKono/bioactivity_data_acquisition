...
## Протоколы
- `BaseApiClient` описывает минимальный HTTP-контракт (`request(method, path, ...)`, `get_json`, `paginate_json`, `close`).
...
- `BaseChemblClient` остаётся чистым транспортом: он адаптирует любой `BaseApiClient`, добавляя обёртку для логирования и единый интерфейс `request`/`get_json`.
