...
## Де-факто файловая политика для `clients`
- **Что допускается**: базовая инфраструктура клиентов (протоколы, HTTP-обёртки, пагинация, нормализаторы, исключения), конкретные клиенты внешних сервисов (ChEMBL, Crossref, OpenAlex, PubChem и др.), утилиты, обслуживающие работу клиентов (общие конверторы payload, логирование на уровне клиента, совместимость/алиасы).
- **Чего быть не должно**: бизнес-логики пайплайнов, нормализации выходов под CSV/ETL, CLI и orchestration-кода, конфигураций пайплайнов, кэширования/стейта уровня ETL. Всё, что не связано напрямую с обращением к внешнему источнику и формированием единых клиентских интерфейсов, переносится в соответствующие пакеты конвейера.
