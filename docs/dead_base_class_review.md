...
| `IParser` и `INormalizer` из `src/bioetl/base_classes.py` | Не имели наследников/использований в актуальных модулях, дублировались в `bioetl.public`, где оставались лишь ради гипотетической обратной совместимости. Удалены из `base_classes` и публичного API. | Средняя |
...
- При удалении `IParser`/`INormalizer` риск затронуть внешних потребителей минимален, но возможны падения при статической типизации у сторонних проектов, которые импортировали эти протоколы напрямую.
...
5. Удалить устаревшие протоколы `IParser` и `INormalizer` из `bioetl.base_classes` и `bioetl.public`, свернуть реэкспорт из `bioetl.__init__`.
6. Добавить миграционную заметку: при необходимости использовать конкретные парсеры/нормализаторы без абстрактных протоколов или объявлять локальные Protocol в проектах-потребителях.
...
- Для удаления `IParser`/`INormalizer` добавить простой smoke-тест импорта `bioetl` и проверки отсутствия атрибутов, чтобы защититься от неявного возврата протоколов в будущем.
