...
### `tests/`
Юнит-, интеграционные и golden-тесты. Структура тестов зеркалирует модули из
`src/`, что упрощает навигацию и поиск примеров использования. Фикстуры и общие
утилиты тестов собираются в подпакетах `tests/conftest` и `tests/utils`.
...
1. **Долгоживущие артефакты** (схемы, стабильные фикстуры) размещаются в
   поддиректориях `data/` или `tests/` и должны быть детерминированы.
