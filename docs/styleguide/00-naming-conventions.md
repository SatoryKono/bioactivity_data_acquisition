...
- `entity`: Domain entity name (`activity`, `assay`, `target`, `document`,
  `testitem`).
- `source`: Data source identifier (`chembl`, `uniprot`, `iuphar`, `pubchem`,
  `pubmed`, `crossref`, `openalex`, `semantic-scholar`).
...
- File H1 heading **MUST** duplicate the file name in title case:
  `# 09 Document ChEMBL Extraction`.
...
Documents that are part of a logical sequence or represent a chapter in a larger
guide **SHOULD** be prefixed with a two-digit number (`NN-`). This ensures they
are ordered correctly in file listings and navigation.
...
Primary index files or landing pages for a directory **SHOULD** be named
`INDEX.md` or use an all-caps name that clearly describes their purpose. They
**SHOULD NOT** have a numeric prefix.
...
`README.md` files **SHOULD** be used as landing pages for specific
subdirectories, especially within `docs/pipelines/sources/`, to provide a brief
overview of the contents.

- **Format**: `README.md`
- **Example**: `docs/pipelines/10-pipelines-catalog.md` (Source documentation
  was consolidated into the main catalog)
