...
- `bioetl.config.models`: 0% - гигантская моно-файловая модель конфигов (~550 строк). Требуются целевые тесты на валидаторы, разнесение по подсекциям, smoke-инициализация.
