...
1. YAML файл + `extends` формируют базовый слой.
...
Если `BIOETL_ENV` отсутствует в окружении/файлах, environment-слой пропускается.
...
- `load_environment_settings()` — читает `.env` (если присутствует) и валидирует
  значения.
- `build_env_override_mapping()` — возвращает чистый словарь overrides, готовый к
  слиянию перед CLI.
- `resolve_env_layers()` — находит файлы `configs/env/<env>` без побочных эффектов.
- `apply_runtime_overrides()` — (опционально) синхронизирует короткие переменные с
  процессным окружением для обратной совместимости.
