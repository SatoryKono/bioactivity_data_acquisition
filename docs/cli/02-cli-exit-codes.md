...
The resilient HTTP layer retries transient errors before surfacing them as `requests.exceptions.HTTPError`. Once the exception propagates, the CLI converts it into exit code `3` with the message prefix `External API failure`. The table below highlights the most common status codes:
...
```text
$ python -m bioetl.cli.app document_chembl --config configs/pipelines/document/document_chembl.yaml --validate-columns
...
...
- `if ! python -m bioetl.cli.app activity_chembl ...; then echo "Pipeline failed!"; exit 1; fi`
...
- **Exit code `2`** indicates a problem with how the CI script is _invoking_ the command. This is a script-level bug and should be fixed in the CI configuration.
- **Exit code `3`** indicates the CLI exhausted its retry budget against an external API. Retry the job after the upstream dependency stabilises or investigate connectivity.
