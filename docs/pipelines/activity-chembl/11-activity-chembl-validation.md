...
This document describes the validation stage of the Activity (ChEMBL) pipeline, covering Pandera schema definitions, field constraints, and validation workflows.
...
- **Measurement Fields**: `standard_type`, `standard_value`, `standard_units`, `standard_relation`
- **Optional Fields**: `pchembl_value`, `data_validity_comment`, `potential_duplicate`

## Validation Workflow

1. **Schema Validation**: Validates against ActivitySchema with strict=True
2. **Duplicate Detection**: Ensures duplicate-free `activity_id` values
3. **Foreign Key Integrity**: Validates references to assays and molecules
4. **Measurement Validity**: Checks numeric ranges and unit consistency
...
- [13-activity-chembl-determinism.md](13-activity-chembl-determinism.md) — Determinism
