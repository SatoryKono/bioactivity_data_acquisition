...
This document describes the validation stage of the Target (ChEMBL) pipeline,
...
1. **Duplicate Detection**: Ensures duplicate-free `target_chembl_id` values
1. **UniProt Validation**: Validates UniProt accession formats
1. **Referential Integrity**: Checks target references in related tables
