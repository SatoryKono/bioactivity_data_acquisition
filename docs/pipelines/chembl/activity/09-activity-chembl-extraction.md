...
1. **Все запросы** через `UnifiedAPIClient` с таймаутами, ретраями и
   ...
   - Handshake `/status` для фиксации `chembl_release` выполняется тем же
     клиентом,
...
> ℹ️ **Важно:** Python-вызов `ChemblActivityPipeline.extract(activity_ids=...)`
> удалён. Передавайте списки идентификаторов через стандартную CLI-опцию
> `--input-file`, чтобы включить batch-режим.
