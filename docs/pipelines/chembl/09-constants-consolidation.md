...
| Лог-события `page_fetched`, `next_link_resolved`, метрики пагинации | `chembl/common/descriptor.py`, `clients/client_chembl_common.py`, `clients/http/pagination.py` | Обновлённый `Paginator` + mixin событий | Все HTTP клиенты будут эмитить одинаковые структурные события через `UnifiedLogger`. |
