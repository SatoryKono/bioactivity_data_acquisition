...
**Новые поля ChEMBL DOCS (v2.0):**

Пайплайн теперь извлекает полные данные из ChEMBL API согласно спецификации DOCS:

- `CHEMBL.DOCS.document_chembl_id` → ChEMBL ID документа (PK)
- `CHEMBL.DOCS.doc_type` → тип документа (PUBLICATION, DATASET, etc.)
- `CHEMBL.DOCS.title` → заголовок публикации/набора (≤ 4000 символов)
- `CHEMBL.DOCS.journal` → название журнала (≤ 255 символов)
- `CHEMBL.DOCS.year` → год публикации (1800-2100)
- `CHEMBL.DOCS.volume` → том (≤ 50 символов)
- `CHEMBL.DOCS.issue` → номер выпуска (≤ 50 символов)
- `CHEMBL.DOCS.first_page` → первая страница (≤ 20 символов)
- `CHEMBL.DOCS.last_page` → последняя страница (≤ 20 символов)
- `CHEMBL.DOCS.doi` → DOI (валидный паттерн)
- `CHEMBL.DOCS.pubmed_id` → PubMed ID (≥ 1)
- `CHEMBL.DOCS.abstract` → аннотация (≤ 4000 символов)
- `CHEMBL.DOCS.chembl_release` → связь на релиз ChEMBL (JSON)

**Новые поля ChEMBL SOURCE (v2.0):**

Данные об источниках документов из ChEMBL API:

- `CHEMBL.SOURCE.src_id` → уникальный ID источника (≥ 1)
- `CHEMBL.SOURCE.src_description` → описание источника (≤ 4000 символов)
- `CHEMBL.SOURCE.src_short_name` → короткое имя (≤ 255 символов)
- `CHEMBL.SOURCE.src_url` → URL источника
- `CHEMBL.SOURCE.data` → полные данные источника (JSON)

**Обогащённые поля (внешние источники):**
...
4. **Новые поля DOCS** (`CHEMBL.DOCS.*`) - полные данные из ChEMBL API
5. **Новые поля SOURCE** (`CHEMBL.SOURCE.*`) - данные об источниках
6. Валидационные поля (`valid_*`, `invalid_*`)

### Новые нормализаторы (v2.0)

Пайплайн включает специализированные нормализаторы для новых полей:

**JSON нормализаторы:**
- `normalize_json_keys()` - нормализация ключей JSON объектов (для `CHEMBL.DOCS.chembl_release`)
- `normalize_json_structure()` - валидация и нормализация JSON структуры (для `CHEMBL.SOURCE.data`)

**Специализированные нормализаторы:**
- `normalize_pmid()` - нормализация PubMed ID с валидацией диапазона
- `normalize_year()` - нормализация года с проверкой диапазона 1800-2100
- `normalize_month()` - нормализация месяца с проверкой диапазона 1-12
- `normalize_day()` - нормализация дня с проверкой диапазона 1-31
