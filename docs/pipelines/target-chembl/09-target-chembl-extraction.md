...
**Расположение конфига:** `configs/pipelines/chembl/target_chembl.yaml`
...
```yaml
# configs/pipelines/chembl/target_chembl.yaml
...
Полный пример конфигурационного файла для target pipeline доступен в `configs/pipelines/chembl/target_chembl.yaml`. Конфигурация включает все необходимые секции для работы пайплайна с детерминизмом, валидацией и извлечением данных из ChEMBL.
...
```json
{
  ...
  "stage": "bootstrap",
  "pipeline": "target",
  "config_path": "configs/pipelines/chembl/target_chembl.yaml",
