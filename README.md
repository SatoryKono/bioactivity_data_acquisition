...
## Проверки качества и статический анализ

Чистка мёртвого кода и контроль регрессий требуют воспроизводимых команд.
После установки `pip install -e .[dev]` доступны следующие цели:

```bash
# запустить минимальный набор тестов с расширенным отчётом
make test

# быстро проверить стиль и ошибки линтера (ruff)
make lint.ruff

# строгий mypy-профиль для src/bioetl и скриптов
make typecheck

# собрать отчёт Vulture о потенциально мёртвом коде
make deadcode
```

`make qa` объединяет форматирование, линтеры, тесты и mypy и служит локальным
эквивалентом нового quality job в GitHub Actions.
