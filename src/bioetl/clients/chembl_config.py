...
class EntityConfig:
    """Декларативный контракт доступа к конкретной ChEMBL-сущности.

    Объект описывает все параметры, необходимые фабрике и клиентам для
    детерминированной работы с endpoint: относительный путь, ключи JSON-ответов,
    политика пагинации, сортировки и фильтров. Каждый специализированный клиент
    получает экземпляр `EntityConfig` из реестра и использует его для построения
    URL, chunking правил, логирования и пост-обработки DataFrame.

    Parameters
    ----------
    endpoint : str
        Относительный путь к endpoint API (например, ``"/activity.json"``).
        Значение служит базой для проверки длины URL (см. `base_endpoint_length`).
    id_field : str
        Имя поля идентификатора в ответе API (используется в логах и QC).
    filter_param : str
        Имя query-параметра, куда подставляются чанки ID (``"activity_id__in"``).
    items_key : str
        Ключ верхнего уровня в JSON, содержащий массив записей (``"activities"``).
    log_prefix : str
        Строка, добавляемая ко всем лог-событиям сущности.
    default_fields : Sequence[str], optional
        Колонки по умолчанию. Используются для формирования пустых DataFrame и
        параметра ``only`` при отсутствии явного списка полей.
    supports_list_result : bool, optional
        Флаг для сущностей, где API возвращает массив без пагинации (редкий кейс).
    chunk_size : int, optional
        Максимальное количество идентификаторов в одном запросе. Проверяется
        при построении чанков и при необходимости уменьшается.
    max_page_size : int | None, optional
        Верхняя граница размера страницы, которую клиенты не должны превышать.
    ordering : Sequence[str], optional
        Список колонок сортировки, гарантирующий стабильный порядок строк.
    filters : Mapping[str, Any], optional
        Фильтры по умолчанию (например, ``{"status": "ACT"}``). Хранятся в
        отсортированном неизменяемом mapping для повторного использования.
    base_endpoint_length : int | None, optional
        Используется при проверке длины URL: если задано, трактуется как длина
        «статической» части пути/параметров.
    enable_url_length_check : bool, optional
        Включает дополнительную проверку, ограничивающую суммарную длину URL.
    dedup_priority : DedupStrategy | None, optional
        Пользовательская функция, разрешающая конфликты в случае дубликатов записей.

    Raises
    ------
    TypeError
        Если строковые поля заданы неверных типов или поля chunk/page-size не int.
    ValueError
        Если строковые поля пусты либо числовые параметры <= 0.
