...
def join_activity_with_molecule(
    ...
    df_result = _perform_joins(
        df_act, compound_records_dict, molecules_dict, log
    )
...
def _fetch_activity_by_ids(
    ...
    try:
        for record in activity_client.iterate_by_ids(
            unique_ids, select_fields=fields
        ):
...
def _perform_joins(
    ...
    if "molecule_chembl_id" in df_act_normalized.columns:
        ...
        if (
            "molecule_chembl_id" in df_molecule.columns
            and not df_molecule.empty
        ):
            df_molecule["molecule_chembl_id"] = df_molecule[
                "molecule_chembl_id"
            ].astype(str)
            df_molecule.loc[
                df_molecule["molecule_chembl_id"] == "nan",
                "molecule_chembl_id",
            ] = pd.NA
    ...
    if (
        "molecule_name" in df_result.columns
        and "molecule_key" in df_result.columns
    ):
    ...
    available_output_cols = [
        col for col in output_columns if col in df_result.columns
    ]
...
def _find_first_nonempty_synonym_in_sequence(
