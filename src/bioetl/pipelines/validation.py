"""Composable validation chain for :class:`bioetl.pipelines.base.PipelineBase`."""
...
from bioetl.core.io import ensure_hash_columns
from bioetl.core.logging import LogEvents, UnifiedLogger
from bioetl.core.schema import format_failure_cases, summarize_schema_errors
from bioetl.schemas import SchemaMigration, SchemaRegistryEntry
...
class ValidationContext:
    ...
    vocabulary_failures_details: list[dict[str, object]] = field(default_factory=list)
...
class FailOpenValidation:
    ...
    def handle_schema_errors(
        ...
        if schema_entry is not None:
            payload.update(schema=schema_entry.identifier, version=schema_entry.version)
        if failure_details:
    ...
    def handle_vocabulary_violations(
        ...
        schema_entry = context.schema_entry
        context.schema_valid = False
        total_invalid = sum(violation.invalid_count for violation in violations)
...
class ValidationStep(Protocol):
    ...
    def run(self, context: ValidationContext, behavior: ValidationBehavior) -> ValidationResult: ...
...
class SchemaResolutionStep:
    ...
    def run(self, context: ValidationContext, behavior: ValidationBehavior) -> ValidationResult:
        schema_identifier = getattr(context.pipeline.config.validation, "schema_out", None)
        ...
        allow_migration = bool(
            getattr(context.pipeline.config.validation, "allow_schema_migration", False)
        )
        schema_entry, migrations, source_version = context.pipeline._resolve_schema_entry(
            schema_identifier,
...
class SchemaValidationStep:
    ...
    def run(self, context: ValidationContext, behavior: ValidationBehavior) -> ValidationResult:
        ...
        df_for_validation = context.pipeline._ensure_load_meta_ids(df_for_validation)
...
class VocabularyValidationStep:
    ...
    def run(self, context: ValidationContext, behavior: ValidationBehavior) -> ValidationResult:
        ...
        violations = context.pipeline._check_vocabulary_bindings(context.df, context.schema_entry)
        if not violations:
            return ValidationResult(df=context.df)
...
class SummaryStep:
    ...
    def run(self, context: ValidationContext, behavior: ValidationBehavior) -> ValidationResult:
        ...
        if context.vocabulary_failures_details:
            summary["vocabulary_failures"] = context.vocabulary_failures_details

        context.pipeline._validation_schema = context.schema_entry
        context.pipeline._validation_schema_version = context.schema_entry.version
