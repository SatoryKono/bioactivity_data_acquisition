...
class GtpIupharPipeline(PipelineBase):
    ...
    def __init__(self, config: PipelineConfig, run_id: str):
        ...
        self.iuphar_client: UnifiedAPIClient | None = None
        if source_config.enabled:
            api_client_config = factory.create("iuphar", source_config)
            self.iuphar_client = UnifiedAPIClient(api_client_config)
            self.register_client(self.iuphar_client)

        page_size = int(source_config.batch_size) if source_config.batch_size else self._IUPHAR_PAGE_SIZE
    ...
    def extract(self, input_file: Path | None = None) -> pd.DataFrame:  # noqa: D401 - part of PipelineBase contract
        if self.iuphar_client is None or not self.source_config.enabled:
            logger.warning(
        ...
        targets = self.iuphar_service.fetch_targets()
        families = self.iuphar_service.fetch_families()
