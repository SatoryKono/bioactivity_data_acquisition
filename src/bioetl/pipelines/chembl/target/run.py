...
class ChemblTargetPipeline(FlattenNestedMixin, UnifiedPipelineBase):
    ...
    def _extract_protein_component_ids_from_flat(
        self, df: pd.DataFrame
    ) -> dict[str, list[int]]:
        """Extract PROTEIN component_id values from target_components__flat column.

        Parses the header_rows format and filters only PROTEIN components.
        ...
        -------
        dict[str, list[int]]:
            Mapping from target_chembl_id to list of PROTEIN component_id values.
        """
        result: dict[str, list[int]] = {}
        ...
        for _, row in df.iterrows():
            ...
            try:
                ...
                headers = parts[0].split("|")
                # Find component_id and component_type column indices
                component_id_idx = None
                component_type_idx = None
                for idx, header in enumerate(headers):
                    header_stripped = header.strip()
                    if header_stripped == "component_id":
                        component_id_idx = idx
                    elif header_stripped == "component_type":
                        component_type_idx = idx

                if component_id_idx is None:
                    continue

                # Extract component_id from each row, filter only PROTEIN
                for row_part in parts[1:]:
                    if not row_part:
                        continue
                    row_values = row_part.split("|")
                    
                    # Check component_type if available
                    if component_type_idx is not None and component_type_idx < len(row_values):
                        component_type_str = row_values[component_type_idx].strip()
                        component_type_str = component_type_str.replace("\\|", "|").replace("\\/", "/").replace("\\\\", "\\")
                        if component_type_str.upper() != "PROTEIN":
                            continue
    ...
    def _enrich_protein_classifications(self, df: pd.DataFrame, log: Any) -> pd.DataFrame:
        ...
        # Extract PROTEIN component_ids from already fetched target_components__flat
        # This avoids the first API call to /target_component.json since component_type
        # is already available in target_components__flat
        protein_component_ids_map = self._extract_protein_component_ids_from_flat(df)

        for target_id in target_ids_to_enrich:
            try:
                # Get PROTEIN component_ids from already fetched data
                # Since each target has only 1 component, we can skip the API call
                protein_component_ids: list[int] = protein_component_ids_map.get(target_id, [])
