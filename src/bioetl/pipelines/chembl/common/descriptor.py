...
class ChemblExtractionDescriptor:
    ...
    def __post_init__(self) -> None:
        if self.mode not in {"chembl", "all"}:
            raise ConfigValidationError("mode must be either 'chembl' or 'all'")
...
class ChemblPipelineContract(Protocol):
    ...
    def resolve_chembl_release(self, config: PipelineConfig) -> tuple[str | None, dict]:
        ...
...
def descriptor_from_csv(
    ...
    *,
    ...
    return ChemblExtractionDescriptor(
        ids=_sanitize_ids(ids), pagination=None, mode=mode, batch_plan=plan, release=release
    )
...
def descriptor_from_options(
    *,
    ...
    plan = BatchPlan(batch_size=batch_size, chunk_size=chunk_size)
    return ChemblExtractionDescriptor(
        ids=_sanitize_ids(ids), pagination=pagination, mode=mode, batch_plan=plan, release=release
    )
