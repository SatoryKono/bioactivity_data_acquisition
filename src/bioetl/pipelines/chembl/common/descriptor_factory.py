...
from .descriptor import BatchPlan
    ...
    def _ensure_factory(self) -> ChemblEntityClientFactory:
        ...
        if self._built_factory is None:
            if self.transport_factory is None:
                msg = "transport_factory is required when client_factory is not provided"
...
class ChemblDescriptorFactory:
    ...
    def __init__(
        ...
        *,
        ...
        self._sort_fields = {k: tuple(v) for k, v in (sort_fields or {}).items()}
    ...
    def build(
        ...
        *,
        ...
        def finalizer_factory(context: Mapping[str, Any]):
            release = context.get("chembl_release") or self._context.chembl_release
            ...
            def finalize(df):
                ...
                if df.empty:
                    return df
                return df.sort_values(by=list(columns_to_sort), ignore_index=True)
