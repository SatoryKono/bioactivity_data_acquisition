    ...
    def run(
        ...
        for batch in batches:
            ...
            if isinstance(result, tuple) and len(result) == 2 and isinstance(result[1], Mapping):
                batch_df = (
                    result[0] if isinstance(result[0], pd.DataFrame) else pd.DataFrame(result[0])
                )
                meta = result[1]
            else:
                batch_df = result if isinstance(result, pd.DataFrame) else pd.DataFrame(result)

            frames.append(batch_df)
            meta = meta or {}
            api_calls += int(meta.get("api_calls", 0 if meta.get("cache_hit") else 1))
            cache_hits += int(meta.get("cache_hit", False)) * max(batch_df.shape[0], 1)
