...
from infrastructure.chembl.normalize import normalize_identifiers
from infrastructure.http import UnifiedAPIClient
from infrastructure.logging import LogEvents
from application.pipelines.orchestration import RunResult
...
from application.pipelines.specs.mixins.enrichment_engine import EnrichmentScenarioEngine
from application.pipelines.specs.mixins.flatten_nested import (
    FlattenNestedMixin,
    FlattenSpec,
)
...
class LoggingMixin:
    ...
    :class:`application.pipelines.specs.base.PipelineBase`.  The mixin exposes
...
class ReleaseHandshakeMixin:
    ...
    implemented by :class:`application.pipelines.specs.base.PipelineBase`.  Pipelines may
...
class SchemaValidationMixin:
    ...
    pipeline (both are provided by :class:`application.pipelines.specs.base.PipelineBase`).
...
class TransformMixin:
    ...
    :class:`infrastructure.chembl.descriptor.ChemblPipelineBase`) together with
...
class IOArtifactsMixin:
    ...
    Contract
    --------
    Requires ``save_results`` from :class:`application.pipelines.specs.base.PipelineBase`.
