...
Examples:
  ...
  python get_document_data.py --input documents.csv --correlation --config configs/config_documents_full.yaml
    ...
    # Input data
    parser.add_argument("--input", type=Path, required=True, help="Path to CSV file with document data (must contain document_chembl_id, doi, title columns)")

    # Sources
    parser.add_argument("--sources", type=str, help="Comma-separated list of sources to use (chembl,crossref,openalex,pubmed,semantic_scholar)")
    ...
    parser.add_argument("--correlation", action="store_true", help="Enable correlation analysis")
    parser.add_argument("--journal-normalization", action="store_true", help="Enable journal name normalization")
    parser.add_argument("--citation-formatting", action="store_true", help="Enable citation formatting")

    # Output format
    parser.add_argument("--format", choices=["csv", "parquet"], help="Output format (overrides config)")

    # Configuration
    parser.add_argument("--config", type=Path, default=Path("configs/config_documents_full.yaml"), help="Configuration file path")
    ...
    try:
        ...
        if not config.runtime.dry_run:
            output_paths = write_document_outputs(result=result, output_dir=config.io.output.dir, date_tag=date_tag, config=config)
