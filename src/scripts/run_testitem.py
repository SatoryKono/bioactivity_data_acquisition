#!/usr/bin/env python3
"""Script for running testitem pipeline."""
...
app = typer.Typer(help="Run testitem pipeline to extract and transform molecule data")


@app.command()
def main(
    input_file: Path = typer.Option(
        Path("data/input/testitem.csv"),
        "--input-file",
        "-i",
    ...
    """Run testitem pipeline with specified parameters."""
    # Setup logging
    ...
    try:
        ...
        pipeline_config_path = Path("configs/pipelines/testitem.yaml")
        ...
        output_filename = f"testitem_{timestamp}.csv"
        output_path = output_dir / output_filename

        logger.info("output_path", path=output_path)

        # Create pipeline instance
        run_id = f"testitem_{timestamp}"
        pipeline = TestItemPipeline(merged_config, run_id)
