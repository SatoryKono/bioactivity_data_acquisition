#!/usr/bin/env python3
"""Script for running activity pipeline."""
...
def main(
    ...
    """Run activity pipeline with specified parameters."""
    # Setup logging
    ...
    try:
        ...
        pipeline_config_path = Path("configs/pipelines/activity.yaml")
        ...
        output_filename = f"activity_{timestamp}.csv"
        output_path = output_dir / output_filename

        logger.info("output_path", path=output_path)

        # Create pipeline instance
        run_id = f"activity_{timestamp}"
        pipeline = ActivityPipeline(merged_config, run_id)
