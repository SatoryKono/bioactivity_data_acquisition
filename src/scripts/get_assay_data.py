...
Examples:
  ...
  python get_assay_data.py --target CHEMBL231 --limit 100 --config configs/config_assay_full.yaml
    ...
    input_group.add_argument("--input", type=Path, help="Path to CSV/JSON file with assay IDs")
    input_group.add_argument("--target", type=str, help="Target ChEMBL ID for filtering assays")

    # Filters
    parser.add_argument("--filters", type=str, help="Filter profile name (e.g., 'human_single_protein')")

    # Limitations
    parser.add_argument("--limit", type=int, help="Maximum number of assays to process")
    ...
    parser.add_argument("--chembl-release", type=str, help="Specific ChEMBL release to use")

    # Output format
    parser.add_argument("--format", choices=["csv", "parquet"], help="Output format (overrides config)")

    # Configuration
    parser.add_argument("--config", type=Path, default=Path("configs/config_assay_full.yaml"), help="Configuration file path")
    ...
    try:
        ...
        if args.input:
            ...
            result = run_assay_etl(config=config, target_chembl_id=args.target, filters=filters)

        # Write outputs
        if not config.runtime.dry_run:
            output_paths = write_assay_outputs(result=result, output_dir=config.io.output.dir, date_tag=date_tag, config=config)
