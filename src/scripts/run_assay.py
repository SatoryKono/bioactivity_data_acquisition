#!/usr/bin/env python3
"""Script for running assay pipeline."""
...
def main(
    ...
    """Run assay pipeline with specified parameters."""
    # Setup logging
    ...
    try:
        ...
        pipeline_config_path = Path("configs/pipelines/assay.yaml")
        ...
        output_filename = f"assay_{timestamp}.csv"
        output_path = output_dir / output_filename

        logger.info("output_path", path=output_path)

        # Create pipeline instance
        run_id = f"assay_{timestamp}"
        pipeline = AssayPipeline(merged_config, run_id)
