...
Examples:
  ...
  python get_testitem_data.py --input molecules.csv --output-dir results/ --config configs/config_testitem_full.yaml
    ...
    # Input data
    parser.add_argument("--input", type=Path, required=True, help="Path to CSV file with molecule data (must contain molecule_chembl_id or molregno columns)")

    # PubChem options
    parser.add_argument("--disable-pubchem", action="store_true", help="Disable PubChem enrichment")

    # Limitations
    parser.add_argument("--limit", type=int, help="Maximum number of molecules to process")
    parser.add_argument("--timeout", type=float, default=60.0, help="Request timeout in seconds")
    parser.add_argument("--retries", type=int, default=10, help="Number of retry attempts")

    # Cache directories
    parser.add_argument("--cache-dir", type=Path, help="Cache directory for ChEMBL data")
    parser.add_argument("--pubchem-cache-dir", type=Path, help="Cache directory for PubChem data")

    # Configuration
    parser.add_argument("--config", type=Path, default=Path("configs/config_testitem_full.yaml"), help="Configuration file path")

    # Output directory
    parser.add_argument("--output-dir", type=Path, help="Output directory (overrides config)")

    # Date tag
    parser.add_argument("--date-tag", type=str, help="Date tag for output files (default: auto-generated)")

    # Logging
    parser.add_argument("--log-level", choices=["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"], default="INFO", help="Logging level")

    # Dry run
    parser.add_argument("--dry-run", action="store_true", help="Simulate execution without making API calls")
    ...
    try:
        ...
        # Load configuration
        config = load_testitem_config(args.config)
        ...
        if args.output_dir:
            ...
            config.io.output.dir = str(args.output_dir)
        if args.cache_dir:
        ...
        if not config.runtime.dry_run:
            output_paths = write_testitem_outputs(result=result, output_dir=Path(config.io.output.dir), date_tag=date_tag, config=config)
