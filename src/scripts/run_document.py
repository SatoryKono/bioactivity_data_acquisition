#!/usr/bin/env python3
"""Script for running document pipeline."""
...
def main(
    ...
    """Run document pipeline with specified parameters."""
    # Setup logging
    ...
    try:
        ...
        pipeline_config_path = Path("configs/pipelines/document.yaml")
        ...
        output_filename = f"documents_{timestamp}.csv"
        output_path = output_dir / output_filename

        logger.info("output_path", path=output_path)

        # Create pipeline instance
        run_id = f"document_{timestamp}"
        pipeline = DocumentPipeline(merged_config, run_id)
        ...
        if dry_run:
            ...
            typer.echo(f"Column count: {len(df.columns)}")
