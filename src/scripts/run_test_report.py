"""CLI для генерации отчётов pytest/coverage с детерминированными артефактами."""
...
@APP.command("run-test-report")
def run_test_report(  # noqa: PLR0915 - CLI orchestration is inherently sequential
    output_root: Path = typer.Option(
        ...
        dir_okay=True,
        writable=True,
        help="Каталог, куда будут записаны артефакты тестирования.",
    ),
) -> None:
    """Запустить pytest, собрать JSON и XML отчёты, сформировать meta.yaml."""
    ...
    final_root = output_root / folder_name
    tmp_root = output_root / f".{folder_name}-{uuid4().hex}.tmp"
    ...
    business_key_hash = _blake2_digest(
        [pipeline_version.encode("utf-8"), b"|", git_commit.encode("utf-8"), b"|", config_hash.encode("utf-8")]
    )
