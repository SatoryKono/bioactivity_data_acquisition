...
from infrastructure.chembl.mixins import ChemblOptionalStringValueMixin
...
class ChemblReleaseMixin(ChemblOptionalStringValueMixin):
    ...
    - Возвращается :class:`ChemblHandshakeResult`, релиз кешируется через
      ...
      (``record_extract_metadata``) с ``chembl_release``
      и ``requested_at_utc``.
    """
...
def perform_chembl_handshake(
    ...
    *,
    ...
    if not enabled:
        logger.info(
            "chembl_handshake_skipped",
            endpoint=endpoint,
            enabled=False,
        )
        release = _normalize_release(
            getattr(chembl_client, release_attr_fallback, None),
        )
    ...
    if callable(handshake_method):
        try:
            raw_payload = (
                handshake_method()
                if endpoint is None
                else handshake_method(endpoint=endpoint)
            )
        except TypeError:
            raw_payload = (
                handshake_method()
                if endpoint is None
                else handshake_method(endpoint)
            )
    else:
        logger.info(
    ...
    if isinstance(raw_payload, Mapping):
        ...
        logger.info(
            "chembl_handshake_unexpected_payload_type",
            endpoint=endpoint,
