...
from pipelines.base import load_pipeline_config
...
def _add_pipeline_commands(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    for name in sorted(PIPELINE_REGISTRY.keys()):
        parser = subparsers.add_parser(name, help=f"Запуск пайплайна {name}")
        ...
        parser.add_argument("--output", help="Путь вывода (приоритетнее конфига)")
        ...
        parser.set_defaults(pipeline_name=name)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="ChEMBL pipelines runner")
