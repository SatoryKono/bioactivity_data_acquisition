...
def check_api_status(api_key: str | None = None, test_pmid: str = "7154002") -> dict:
    ...
    headers = {"Accept": "application/json", "User-Agent": "bioactivity-data-acquisition/0.1.0"}
    ...
    try:
        ...
        response = requests.get(f"https://api.semanticscholar.org/graph/v1/paper/PMID:{test_pmid}", headers=headers, params=params, timeout=30)
        ...
        if response.status_code == 200:
            try:
                data = response.json()
                result["data_received"] = {"has_title": bool(data.get("title")), "has_abstract": bool(data.get("abstract")), "year": data.get("year")}
...
def test_multiple_requests(api_key: str | None = None, num_requests: int = 5) -> dict:
    ...
    for i in range(min(num_requests, len(test_pmids))):
        ...
        try:
            start_time = time.time()
            response = requests.get(f"https://api.semanticscholar.org/graph/v1/paper/PMID:{pmid}", headers=headers, params={"fields": "title"}, timeout=10)
            response_time = (time.time() - start_time) * 1000

            if response.status_code == 200:
                logger.info(f"[OK] ({response_time:.0f}ms)")
                results.append({"request": i + 1, "pmid": pmid, "status_code": response.status_code, "response_time_ms": response_time, "success": True})
            elif response.status_code == 429:
                logger.error("[ERROR] Rate limited")
                results.append({"request": i + 1, "pmid": pmid, "status_code": response.status_code, "success": False, "error": "Rate limited"})
                break
            else:
                logger.warning(f"[WARN] {response.status_code}")
                results.append({"request": i + 1, "pmid": pmid, "status_code": response.status_code, "success": False, "error": f"Status {response.status_code}"})
            ...
            results.append({"request": i + 1, "pmid": pmid, "success": False, "error": str(e)})
            break
...
def main():
    ...
    if args.test_limits:
        ...
        if "data_received" in result:
            ...
            logger.info(
                f"Данные: Заголовок: {'[OK]' if data.get('has_title') else '[NO]'}, Аннотация: {'[OK]' if data.get('has_abstract') else '[NO]'}, Год: {data.get('year', 'N/A')}"
            )
