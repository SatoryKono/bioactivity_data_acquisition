...
class SemanticScholarMonitor:
    ...
    def test_api_call(self) -> dict[str, Any]:
        ...
        try:
            response = self.session.get(f"{self.base_url}/{test_pmid}", headers=self.headers, params={"fields": "title,abstract,year"}, timeout=30)
            ...
            if response.status_code == 200:
                data = response.json()
                result["data_received"] = {"title": data.get("title", "N/A"), "year": data.get("year", "N/A"), "has_abstract": bool(data.get("abstract"))}
            else:
            ...
            return {"timestamp": datetime.now().isoformat(), "success": False, "error": str(e), "error_type": type(e).__name__}
