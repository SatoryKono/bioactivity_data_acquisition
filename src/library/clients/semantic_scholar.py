...
class SemanticScholarClient(BaseApiClient):
    ...
    def fetch_by_pmid(self, pmid: str) -> dict[str, Any]:
        ...
        try:
            ...
            payload = self._request_with_fallback("GET", identifier, params={"fields": ",".join(self._DEFAULT_FIELDS)})
            ...
            if payload.get("source") == "fallback" or payload.get("fallback_used"):
                ...
                self.logger.warning("semantic_scholar_fallback_used", pmid=pmid, error=error_msg, fallback_reason=fallback_reason_msg)
            ...
            if "429" in str(exc) or "rate limited" in str(exc).lower():
                ...
                self.logger.warning(
                    "semantic_scholar_rate_limited", pmid=pmid, error=error_msg, message=("Rate limited by Semantic Scholar API. Consider getting an API key for higher limits.")
                )
                ...
                # Экранируем символы % в сообщениях об ошибках для безопасного логирования
                error_msg = str(exc).replace("%", "%%")
                self.logger.error("semantic_scholar_request_failed", pmid=pmid, error=error_msg, error_type=type(exc).__name__)
                exc_msg = str(exc).replace("%", "%%")
    ...
    def _parse_paper(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        record: dict[str, Any | None] = {
            ...
            "semantic_scholar_external_ids": (json.dumps(external_ids) if external_ids else None),
