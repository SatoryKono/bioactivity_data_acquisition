...
class GtoPdbClient(BaseApiClient):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.circuit_breaker = CircuitBreaker(failure_threshold=cb_config.get("failure_threshold", 5), holdoff_seconds=cb_config.get("holdoff_seconds", 300))

        rl_config = rate_limit_config or {}
        self.rate_limiter = RateLimiter(rps=rl_config.get("rps", 1.0), burst=rl_config.get("burst", 2))
    ...
    def _build_url(self, gtop_id: str, endpoint: str) -> str:
        ...
        # Use webServices.jsp as the main API endpoint
        return f"{base_url}/webServices.jsp?targetId={gtop_id}&endpoint={endpoint}"
    ...
    def _fetch_endpoint(self, gtop_id: str, endpoint: str) -> list[dict[str, Any]] | None:
        ...
        if self.circuit_breaker.is_open():
            self.logger.warning(f"gtop_circuit_open gtop_id={gtop_id} endpoint={endpoint} failures={self.circuit_breaker._failures}")
            return None
        ...
        try:
            ...
            if response.headers.get("Content-Length") == "0" or not response.text.strip():
                self.logger.info(f"gtop_empty_response gtop_id={gtop_id} endpoint={endpoint} content_type={content_type}")
                self.circuit_breaker.record_success()
                return []

            # Validate JSON response
            if "application/json" not in content_type.lower():
                self.logger.warning(f"gtop_non_json_response gtop_id={gtop_id} endpoint={endpoint} content_type={content_type}")
            ...
            if isinstance(data, list):
                ...
                if "GtoPdb Web Services" in data:
                    self.logger.info(f"gtop_metadata_response gtop_id={gtop_id} endpoint={endpoint} message={data.get('GtoPdb Web Services', '')}")
                    return []
            ...
            if status_code == 404:
                self.logger.info(f"gtop_endpoint_missing gtop_id={gtop_id} endpoint={endpoint} status_code={status_code}")
                self.circuit_breaker.record_success()
                with self._cache_lock:
            ...
            self.logger.warning(f"gtop_request_failed gtop_id={gtop_id} endpoint={endpoint} error={str(exc)} status_code={status_code}")
            ...
            self.logger.warning(f"gtop_json_decode_failed gtop_id={gtop_id} endpoint={endpoint} error={str(exc)}")
