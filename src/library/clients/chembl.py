...
class ChEMBLClient(BaseApiClient):
    ...
    def fetch_documents_batch(
        ...
        for i in range(0, len(document_chembl_ids), batch_size):
            batch_ids = document_chembl_ids[i:i + batch_size]
            
            try:
                # Build URL with document IDs
                ids_str = ",".join(batch_ids)
                url = f"document?document_chembl_id__in={ids_str}&format=json&limit=1000"
                ...
                if isinstance(response_data, dict) and "documents" in response_data:
                    ...
                    if isinstance(documents, list):
                        for document in documents:
                            yield self._parse_document({"documents": [document]})
                    else:
                        logger.warning(f"Unexpected documents format in response: {type(documents)}")
                else:
                    logger.warning(f"Unexpected response format: {type(response_data)}")

            except Exception as e:
                logger.error(f"Failed to fetch documents for batch {i // batch_size + 1}: {e}")
                # Continue with next batch instead of failing completely
                continue

    def _parse_document(self, payload: dict[str, Any]) -> dict[str, Any]:
        """Parse document data from ChEMBL API response."""
        document = payload.get("documents", [{}])[0] if "documents" in payload else payload
