...
class TestitemQualityFilter:
    ...
    def apply_strict_quality_filter(self, df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        required_fields = self.strict_config.get("required_fields", ["molecule_chembl_id"])
        ...
        if "mw_freebase" in df.columns:
            invalid_mw_mask = df["mw_freebase"].notna() & ((df["mw_freebase"] <= 0) | (df["mw_freebase"] > 10000))
            if invalid_mw_mask.any():
                accepted_mask &= ~invalid_mw_mask
                rejection_reasons.extend(["invalid_molecular_weight" for _ in range(invalid_mw_mask.sum())])
        ...
        if "alogp" in df.columns:
            invalid_alogp_mask = df["alogp"].notna() & ((df["alogp"] < -10) | (df["alogp"] > 10))
            if invalid_alogp_mask.any():
                accepted_mask &= ~invalid_alogp_mask
                rejection_reasons.extend(["invalid_alogp" for _ in range(invalid_alogp_mask.sum())])
        ...
        if "molecule_chembl_id" in df.columns:
            ...
            if duplicate_mask.any():
                ...
                rejection_reasons.extend(["duplicate_molecule_chembl_id" for _ in range(duplicate_mask.sum())])
        ...
        if "standardized_smiles" in df.columns:
            ...
            if invalid_smiles_mask.any():
                ...
                rejection_reasons.extend(["invalid_smiles" for _ in range(invalid_smiles_mask.sum())])
