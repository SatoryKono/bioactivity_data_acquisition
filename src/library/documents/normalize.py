...
# from library.schemas.document_schema_normalized import # DocumentNormalizedSchema  # Commented out due to missing module  # Module not found - commented out
...
class DocumentNormalizer:
    ...
    def _normalize_field_types(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # List fields
        list_fields = ["pubmed_authors", "semantic_scholar_authors", "openalex_concepts"]
        ...
        # Dict fields
        dict_fields = ["crossref_subject", "openalex_concepts"]
        ...
        # Boolean fields
        boolean_fields = ["referenses_on_previous_experiments", "original_experimental_document"]
        ...
        for field in boolean_fields:
            if field in normalized_df.columns:
                ...
                normalized_df[field] = (
                    normalized_df[field].astype(str).str.lower().map({"true": True, "false": False, "1": True, "0": False, "yes": True, "no": False}).fillna(False).astype("bool")
                )
    ...
    def _apply_schema_normalizations(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Получаем схему
        # schema = DocumentNormalizedSchema.get_schema()  # Commented out due to missing module
        schema = None  # Placeholder

        # Применяем нормализацию к каждой колонке
        if schema is not None and hasattr(schema, 'columns'):
            for column_name, column_schema in schema.columns.items():  # type: ignore[union-attr]
    ...
    def _add_system_metadata(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        chembl_client = create_api_client("chembl", self.config, "document")
