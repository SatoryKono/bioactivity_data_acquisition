...
def _normalize_dataframe(df: pd.DataFrame, determinism: DeterminismSettings | None = None, logger: BoundLogger | None = None) -> pd.DataFrame:
    ...
    for col, low in lower_names.items():
        ...
        if low in {"doi", "document_doi", "doi_key", "valid_doi", "chembl_doi", "crossref_doi", "openalex_doi", "pubmed_doi", "semantic_scholar_doi"}:
...
def _apply_qc_thresholds(
    ...
    summary_status = "fail" if any(status == "fail" and threshold is not None for status, threshold in zip(statuses, thresholds, strict=False)) else "pass"
    ...
    frames = [df for df in (qc_with_thresholds, summary) if not df.empty]
    if not frames:
        # Вернём пустой фрейм с ожидаемыми колонками
        return pd.DataFrame(columns=["metric", "value", "threshold", "ratio", "status"])
...
def write_deterministic_csv(
    ...
    *,
    ...
    if not df.empty and not _is_report_file(destination):
        _auto_generate_qc_and_correlation_reports(df_to_write, destination, output, postprocess, logger=logger)
...
def _is_qc_report(df: pd.DataFrame) -> bool:
    ...
    return len(df.columns) == 2 and "metric" in df.columns and "value" in df.columns
...
def _auto_generate_qc_and_correlation_reports(
    ...
    if not postprocess.qc.enabled and not postprocess.correlation.enabled:
        if logger is not None:
            logger.info("skip_report_generation", reason="postprocess_disabled", qc_enabled=postprocess.qc.enabled, correlation_enabled=postprocess.correlation.enabled)
        return
    ...
    try:
        ...
        if postprocess.correlation.enabled:
            ...
            # Сохраняем только если корреляционная матрица не пустая
            if not correlation.empty:
                corr_path.parent.mkdir(parents=True, exist_ok=True)
                if file_format == "parquet":
                    correlation.to_parquet(corr_path.with_suffix(".parquet"), index=True, compression=parquet_settings.compression)
                else:
                    # Для корреляционной матрицы нужно сохранять с индексами
                    corr_csv_options = _csv_options(csv_settings).copy()
                    corr_csv_options["index"] = True
                    correlation.to_csv(corr_path, **corr_csv_options)

                if logger is not None:
                    logger.info("auto_corr_basic_saved", path=str(corr_path))
            else:
                if logger is not None:
                    logger.info("auto_corr_basic_skipped", reason="empty_correlation_matrix", path=str(corr_path))
        ...
        if postprocess.qc.enabled and hasattr(postprocess.qc, "enhanced") and postprocess.qc.enhanced:
            ...
            if logger is not None:
                logger.info("auto_qc_enhanced_saved", enhanced_path=str(enhanced_qc_path), detailed_path=str(detailed_qc_path))
        ...
        if postprocess.correlation.enabled and hasattr(postprocess.correlation, "enhanced") and postprocess.correlation.enhanced:
            ...
            if logger is not None:
                logger.info("auto_corr_enhanced_saved", enhanced_path=str(enhanced_corr_path), detailed_path=str(detailed_corr_path))
