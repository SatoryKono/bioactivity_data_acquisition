...
class AssayChEMBLClient(BaseApiClient):
    ...
    def fetch_assays_batch(self, assay_ids: list[str], batch_size: int = 100) -> list[dict[str, Any]]:
        ...
        for i in range(0, len(assay_ids), batch_size):
            ...
            try:
                params = {"assay_chembl_id__in": batch_str, "format": "json", "limit": len(batch)}
    ...
    def fetch_by_target_id(self, target_chembl_id: str, filters: dict[str, Any] | None = None) -> list[dict[str, Any]]:
        """Retrieve assays by target ChEMBL identifier with optional filters."""

        params = {"target_chembl_id": target_chembl_id, "format": "json", "limit": 200}
    ...
    def fetch_source_info(self, src_id: int) -> dict[str, Any]:
        ...
        try:
            ...
            return {"src_id": src_id, "src_name": response.get("src_description"), "src_short_name": response.get("src_short_name"), "src_url": response.get("src_url")}
        except Exception as e:
            logger.warning(f"Failed to fetch source {src_id}: {e}")
            return {"src_id": src_id, "src_name": None, "src_short_name": None, "src_url": None}
    ...
    def get_chembl_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"chembl_release": response.get("chembl_release"), "status": response.get("status"), "timestamp": datetime.utcnow().isoformat()}
        except Exception as e:
