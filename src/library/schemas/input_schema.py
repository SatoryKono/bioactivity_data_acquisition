...
class RawBioactivitySchema(pa.DataFrameModel):
    ...
    source: Series[str] = pa.Field(description="Data source identifier (e.g., 'chembl', 'crossref')", nullable=False)
    ...
    target_pref_name: Series[str] = pa.Field(description="Preferred name of the biological target", nullable=True)
    standard_value: Series[float] = pa.Field(description="Bioactivity value (IC50, Ki, etc.)", nullable=True)
    standard_units: Series[str] = pa.Field(description="Units for the bioactivity value (nM, uM, etc.)", nullable=True)
    canonical_smiles: Series[str] = pa.Field(description="Canonical SMILES representation of the compound", nullable=True)

    # ChEMBL-specific fields - nullable because not all sources provide them
    activity_id: Series[int] = pa.Field(description="Unique activity identifier from ChEMBL", nullable=True)
    ...
    standard_type: Series[str] = pa.Field(description="Type of bioactivity measurement (IC50, Ki, etc.)", nullable=True)
    standard_relation: Series[str] = pa.Field(description="Relation to the standard value (=, <, >, etc.)", nullable=True)
    target_chembl_id: Series[str] = pa.Field(description="ChEMBL target identifier", nullable=True)
    target_organism: Series[str] = pa.Field(description="Organism of the biological target", nullable=True)
    target_tax_id: Series[str] = pa.Field(description="Taxonomic identifier of the target organism", nullable=True)
