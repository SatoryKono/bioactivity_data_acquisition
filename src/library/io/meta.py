...
class DatasetMetadata:
    ...
    def get_chembl_status(self, config: APIClientConfig | None = None) -> dict[str, Any]:
        ...
        if self._chembl_status is None:
            if config is None:
                ...
                config = APIClientConfig(name="chembl", base_url="https://www.ebi.ac.uk/chembl/api/data", timeout=30, retries=RetrySettings(total=3))

            try:
                client = ChEMBLClient(config)
                # self._chembl_status = client.get_chembl_status()  # Method not available
                # Use fetch method to get status
                try:
    ...
    def to_dict(self, config: APIClientConfig | None = None) -> dict[str, Any]:
        ...
        meta = {
            "pipeline": {"name": self.dataset, "version": "2.0.0", "entity_type": self.dataset, "source_system": "chembl"},
            "execution": {"run_id": self._run_id, "started_at": self._started_at, "completed_at": self._completed_at, "duration_sec": self._duration_sec},
            "data": {
...
def create_dataset_metadata(dataset_name: str = "chembl", config: APIClientConfig | None = None, logger: BoundLogger | None = None) -> DatasetMetadata:
