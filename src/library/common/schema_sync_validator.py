...
class SchemaSyncValidator:
    ...
    def validate_all_schemas(self, configs_dir: Path) -> bool:
        ...
        if doc_config_path.exists():
            try:
                ...
                is_valid = self.validate_entity_schema_sync("documents", doc_config_path, DocumentNormalizedSchema.get_schema())
        ...
        if testitem_config_path.exists():
            try:
                ...
                is_valid = self.validate_entity_schema_sync("testitems", testitem_config_path, TestitemNormalizedSchema.get_schema())
                all_valid = all_valid and is_valid
            except ImportError as e:
    ...
    def get_validation_report(self) -> dict[str, Any]:
        ...
        """
        return {"errors": self.errors, "warnings": self.warnings, "has_errors": len(self.errors) > 0, "has_warnings": len(self.warnings) > 0, "strict_mode": self.strict_mode}
