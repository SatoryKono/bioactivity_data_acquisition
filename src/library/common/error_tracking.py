...
class ErrorTracker:
    ...
    def add_error(
        ...
        error = ErrorInfo(error_type=error_type, severity=severity, source=source, message=message, details=details, record_id=record_id, stack_trace=stack_trace)
...
class DataFrameErrorTracker:
    ...
    def add_extraction_errors_column(self, df: pd.DataFrame, source: str, errors: dict[str, str]) -> pd.DataFrame:
        ...
        for record_id, error_message in errors.items():
            if record_id in df.index:
                ...
                # Добавить в трекер
                self.tracker.add_error(error_type=ErrorType.EXTRACTION_ERROR, source=source, message=error_message, record_id=str(record_id))
    ...
    def add_validation_errors_column(self, df: pd.DataFrame, validation_errors: dict[str, list[str]]) -> pd.DataFrame:
        ...
        for record_id, errors in validation_errors.items():
            if record_id in df.index:
                ...
                # Добавить в трекер
                for error in errors:
                    self.tracker.add_error(error_type=ErrorType.VALIDATION_ERROR, source="validation", message=error, record_id=str(record_id))
    ...
    def add_global_error_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.tracker.errors:
            has_critical_errors = any(error.severity == ErrorSeverity.CRITICAL for error in self.tracker.errors)
        ...
        for error in self.tracker.get_errors_by_type(ErrorType.EXTRACTION_ERROR):
            if error.record_id:
                ...
                extraction_errors[error.record_id].append({"source": error.source, "error_type": error.error_type.value, "message": error.message, "timestamp": error.timestamp})
        ...
        for error in self.tracker.get_errors_by_type(ErrorType.VALIDATION_ERROR):
            if error.record_id:
                ...
                validation_errors[error.record_id].append({"error_type": error.error_type.value, "message": error.message, "timestamp": error.timestamp})
...
def add_error_columns_to_dataframe(
    df: pd.DataFrame, entity_type: str, extraction_errors: dict[str, dict[str, str]] | None = None, validation_errors: dict[str, list[str]] | None = None
) -> pd.DataFrame:
...
def extract_errors_from_dataframe(df: pd.DataFrame) -> dict[str, Any]:
    ...
    errors = {"extraction_errors": {}, "validation_errors": {}, "extraction_status": {}}
