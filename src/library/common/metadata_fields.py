...
def add_system_metadata_fields(
    ...
    if chembl_client:
        try:
            ...
            if chembl_release:
                logger.info(f"Retrieved ChEMBL release from API: {chembl_release}")
    ...
    df_with_metadata['hash_row'] = df_with_metadata.apply(
        lambda row: _calculate_row_hash(row, exclude_hash_fields=True), 
        axis=1
    )
    ...
    if business_key_fields:
        df_with_metadata['hash_business_key'] = df_with_metadata[business_key_fields].apply(
            lambda row: _calculate_business_key_hash(row), 
            axis=1
        )
    else:
...
def create_chembl_client_from_config(config: dict[str, Any]) -> ChEMBLClient | None:
    """Создать ChEMBL клиент из конфигурации.
    
    Args:
        config: Конфигурация пайплайна
        
    Returns:
        ChEMBL клиент или None если не удалось создать
    """
    try:
        chembl_config = config.get('sources', {}).get('chembl', {})
        if not chembl_config.get('enabled', False):
            logger.info("ChEMBL source is disabled, skipping client creation")
            return None
        ...
        logger.warning(f"Failed to create ChEMBL client: {e}")
        return None
