    ...
    def check_thresholds(self, metrics: dict[str, float]) -> dict[str, bool]:
        ...
        for threshold in self.profile.thresholds:
            ...
            if not passed and threshold.fail_on_exceed:
                raise ValueError(
                    f"Порог качества превышен: {threshold.metric} = {metric_value:.3f} "
                    f"(порог: {threshold.threshold:.3f})"
                )
...
class ActivityQCValidator(QCValidator):
    ...
    def validate(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        if "standard_value" in df.columns:
            metrics["invalid_standard_value"] = (
                (df["standard_value"] < 1e-12) | (df["standard_value"] > 1e-3)
            ).sum() / total_rows
        ...
        if "pchembl_value" in df.columns:
            ...
            metrics["invalid_pchembl_value"] = (
                (df["pchembl_value"] < 3.0) | (df["pchembl_value"] > 12.0)
            ).sum() / total_rows
...
class TestitemQCValidator(QCValidator):
    ...
    def validate(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        if "molecular_weight" in df.columns:
            ...
            metrics["invalid_molecular_weight"] = (
                (df["molecular_weight"] < 50.0) | (df["molecular_weight"] > 2000.0)
            ).sum() / total_rows
...
STRICT_PROFILE = QCProfile(
    ...
    rules=[
        QualityRule(
            ...
            description="ChEMBL ID должен соответствовать паттерну ^CHEMBL\\d+$",
            enabled=True
        )
    ]
)
