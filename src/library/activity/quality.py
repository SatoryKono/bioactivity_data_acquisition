...
class ActivityQualityFilter:
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        self.rejected_activity_comments = normalization_config.get('rejected_activity_comments', 
                                                                  ['inconclusive', 'undetermined', 'unevaluated'])
    ...
    def apply_strict_quality_filter(self, df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        required_fields = self.strict_config.get('required_fields', [
            'assay_chembl_id', 'molecule_chembl_id', 'standard_type', 
        ...
        if 'standard_type' in df.columns:
            ...
            if invalid_type_mask.any():
                ...
                rejection_reasons.extend([
                    f"invalid_activity_type_{row['standard_type']}" 
                    for _, row in df[invalid_type_mask].iterrows()
                ])
        ...
        if 'standard_relation' in df.columns:
            ...
            if invalid_relation_mask.any():
                ...
                rejection_reasons.extend([
                    f"invalid_relation_{row['standard_relation']}" 
                    for _, row in df[invalid_relation_mask].iterrows()
                ])
        ...
        if 'data_validity_comment' in df.columns:
            ...
            if has_validity_comment.any():
                ...
                rejection_reasons.extend([
                    f"has_validity_comment_{row['data_validity_comment']}" 
                    for _, row in df[has_validity_comment].iterrows()
                ])
        ...
        if 'activity_comment' in df.columns:
            ...
            if rejected_comment_mask.any():
                ...
                rejection_reasons.extend([
                    f"rejected_activity_comment_{row['activity_comment']}" 
                    for _, row in df[rejected_comment_mask].iterrows()
                ])
    ...
    def apply_moderate_quality_filter(self, df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        if 'data_validity_comment' in df.columns and allowed_warnings:
            invalid_warning_mask = (
                df['data_validity_comment'].notna() & 
                ~df['data_validity_comment'].isin(allowed_warnings)
            )
            if invalid_warning_mask.any():
                accepted_mask &= ~invalid_warning_mask
                rejection_reasons.extend([
                    f"invalid_validity_comment_{row['data_validity_comment']}" 
                    for _, row in df[invalid_warning_mask].iterrows()
                ])
    ...
    def apply_quality_profiles(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        results['strict_quality'] = {
            'accepted': strict_accepted,
            'rejected': strict_rejected,
            'accepted_count': len(strict_accepted),
        ...
        logger.info(f"Quality profiling completed: "
                   f"Strict: {len(strict_accepted)} accepted, "
                   f"Moderate: {len(moderate_accepted)} accepted, "
    ...
    def get_quality_statistics(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        for field in critical_fields:
            if field in df.columns:
                ...
                stats['missing_data'][field] = {
                    'count': int(missing_count),
                    'fraction': float(missing_count / len(df)) if len(df) > 0 else 0.0
                }
        ...
        for field in fk_fields:
            if field in df.columns:
                ...
                stats['foreign_key_coverage'][field] = {
                    'count': int(coverage_count),
                    'fraction': float(coverage_count / len(df)) if len(df) > 0 else 0.0
                }
        ...
        for field, coverage_stats in stats.get('foreign_key_coverage', {}).items():
            if coverage_stats['fraction'] < min_coverage:
                ...
                validation_results['violations'].append({
                    'type': 'low_coverage',
                    'field': field,
                    'actual': coverage_stats['fraction'],
