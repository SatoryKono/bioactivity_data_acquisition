...
from library.documents.config import ALLOWED_SOURCES, DEFAULT_ENV_PREFIX, ConfigLoadError, load_document_config
...
from library.target import TargetQCError, TargetValidationError, load_target_config, write_target_outputs
...
# TODO: Use in future CLI updates
# from library.activity.pipeline_v2 import ActivityPipeline
# from library.assay.pipeline_v2 import AssayPipeline
# from library.documents.pipeline_v2 import DocumentPipeline
# from library.target.pipeline_v2 import TargetPipeline
# from library.testitem.pipeline_v2 import TestitemPipeline
...
# Унифицированные опции для всех команд
UNIFIED_TIMEOUT_OPTION = typer.Option(None, "--timeout", help="HTTP timeout in seconds")
UNIFIED_RETRIES_OPTION = typer.Option(None, "--retries", help="Total retry attempts")
UNIFIED_LIMIT_OPTION = typer.Option(None, "--limit", help="Limit number of records")
UNIFIED_DRY_RUN_OPTION = typer.Option(False, "--dry-run", help="Do not write outputs")
UNIFIED_LOG_LEVEL_OPTION = typer.Option("INFO", "--log-level", help="Logging level")
UNIFIED_LOG_FILE_OPTION = typer.Option(None, "--log-file", help="Path to log file")
UNIFIED_LOG_FORMAT_OPTION = typer.Option("text", "--log-format", help="Console format (text/json)")
UNIFIED_NO_FILE_LOG_OPTION = typer.Option(False, "--no-file-log", help="Disable file logging")
UNIFIED_INPUT_OPTION = typer.Option(None, "--input", "-i", help="Input CSV file", resolve_path=True)
UNIFIED_OUTPUT_DIR_OPTION = typer.Option(None, "--output-dir", "-o", help="Output directory", resolve_path=True)
UNIFIED_DATE_TAG_OPTION = typer.Option(None, "--date-tag", help="YYYYMMDD date tag")
UNIFIED_WORKERS_OPTION = typer.Option(None, "--workers", help="Number of worker threads")
UNIFIED_VERBOSE_OPTION = typer.Option(False, "--verbose", "-v", help="Enable verbose logging")

# Обратная совместимость (deprecated)
DEFAULT_TIMEOUT_OPTION = typer.Option(None, "--timeout-sec", help="[DEPRECATED] Use --timeout instead")
...
TARGET_INPUT_OPTION = typer.Option(..., "--input", help="CSV containing target_chembl_id column", resolve_path=True)
...
ACTIVITY_INPUT_OPTION = typer.Option(None, "--input", help="CSV containing filter IDs (assay_ids, molecule_ids, target_ids)", resolve_path=True)
...
def _normalise_sources(raw_sources: Iterable[str]) -> list[str]:
    ...
    for item in raw_sources:
        ...
        if key not in ALLOWED_SOURCES:
            raise ValueError(f"Unsupported source '{item}'. Valid options: {', '.join(ALLOWED_SOURCES)}")
...
def _build_cli_overrides(
    *,
    ...
    timeout_sec: float | None = None,  # Deprecated
    retries: int | None,
    workers: int | None,
    limit: int | None,
    sources: list[str],
    ...
    # Обработка deprecated флагов с предупреждением
    if timeout_sec is not None:
        typer.echo("Warning: --timeout-sec is deprecated, use --timeout instead", err=True)
        timeout = timeout_sec
    ...
    if timeout is not None:
        _assign_path(overrides, ["http", "global", "timeout_sec"], timeout)
...
def get_target_data(
    *,
    ...
    input: Path = UNIFIED_INPUT_OPTION,
    output_dir: Path | None = UNIFIED_OUTPUT_DIR_OPTION,
    date_tag: str | None = UNIFIED_DATE_TAG_OPTION,
    timeout: float | None = UNIFIED_TIMEOUT_OPTION,
    timeout_sec: float | None = DEFAULT_TIMEOUT_OPTION,  # Deprecated
    retries: int | None = UNIFIED_RETRIES_OPTION,
    limit: int | None = UNIFIED_LIMIT_OPTION,
    dev_mode: bool = TARGET_DEV_MODE_OPTION,
    dry_run: bool = UNIFIED_DRY_RUN_OPTION,
    log_level: str = UNIFIED_LOG_LEVEL_OPTION,
    log_file: Path | None = UNIFIED_LOG_FILE_OPTION,
    log_format: str = UNIFIED_LOG_FORMAT_OPTION,
) -> None:
    ...
    with bind_stage(logger, "target_processing", run_id=run_id) as logger:
        ...
        try:
            ...
            logger.error("Target processing failed", error=str(exc), run_id=run_id, exc_info=True)
            typer.echo(str(exc), err=True)
...
def get_activity_data(
    *,
    ...
    try:
        ...
        if timeout is not None:
            activity_config.timeout_sec = timeout
        if retries is not None:
        ...
        try:
            with ShutdownContext(timeout=60.0):
                with bind_stage(logger, "activity_etl"):
                    pipeline = ActivityPipeline(activity_config)
                    result = pipeline.run(input_data=input)

            logger.info("Activity processing completed", run_id=run_id, records=len(result.data))
        ...
        try:
            with ShutdownContext(timeout=60.0):
                with bind_stage(logger, "document_etl"):
                    pipeline = DocumentPipeline(config_model)
                    result = pipeline.run(input_data=input_frame)

            logger.info("Document processing completed", run_id=run_id, records=len(result.data))
...
def testitem_run(
    ...
    input: Path = UNIFIED_INPUT_OPTION,
    output: Path = UNIFIED_OUTPUT_DIR_OPTION,
    cache_dir: Path = TESTITEM_CACHE_DIR_OPTION,
    pubchem_cache_dir: Path = TESTITEM_PUBCHEM_CACHE_DIR_OPTION,
    timeout: int = UNIFIED_TIMEOUT_OPTION,
    retries: int = UNIFIED_RETRIES_OPTION,
    limit: int = UNIFIED_LIMIT_OPTION,
    disable_pubchem: bool = TESTITEM_DISABLE_PUBCHEM_OPTION,
    dry_run: bool = UNIFIED_DRY_RUN_OPTION,
    verbose: bool = UNIFIED_VERBOSE_OPTION,
    date_tag: str = UNIFIED_DATE_TAG_OPTION,
) -> None:
    ...
    try:
        ...
        console.print(f"  Processed {len(result.data)} molecules")
        ...
        output_paths = write_testitem_outputs(result, output, testitem_config.runtime.date_tag, testitem_config)
...
def install_completion(shell: str = typer.Argument(..., help="Shell type (bash, zsh, fish, powershell)")) -> None:
    ...
    try:
        ...
        cmd = [executable, "-m", "typer", "library.cli:app", f"--name={script_name}", shell, "--output-file", "-"]
