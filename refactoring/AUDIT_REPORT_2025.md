...
| P0-6 | Устаревшие ссылки на конфиги | ✅ **ИСПРАВЛЕНО** | Ссылки обновлены: `PIPELINES.md`, `MODULE_RULES.md`, `IO.md`, `DATA_SOURCES.md`, `REFACTOR_PLAN.md` используют путь `src/bioetl/configs/pipelines/` |
...
| P0-6 | Устаревшие ссылки на конфиги | ✅ **ИСПРАВЛЕНО** | Подтверждено: все документы указывают на `src/bioetl/configs/pipelines/` |
...
- Приоритизировать исправление оставшихся P0 проблем (например, P0-1: решить дублирование ChEMBL пайплайнов)
...
- ✅ Устаревшие ссылки (`@test_refactoring_11`, `@Pipeline_Unification`) исправлены на `@test_refactoring_32`
- ✅ **P0-6 (ссылки на конфиги)**: Множественные упоминания приведены к `src/bioetl/configs/pipelines/`
...
- ✅ **ИСПРАВЛЕНО:** P0-6 (ссылки на конфиги)
...
1. **`PIPELINES.md`** — обновить структуру источников:
   - Заменить упоминания `src/bioetl/configs/pipelines/<source>/pipeline.yaml` на `src/bioetl/configs/pipelines/<source>.yaml` (в соответствии с фактическим расположением конфигов)
...
2. **`IO.md`** — исправить пути к конфигам:
   - Заменить все упоминания `src/bioetl/configs/pipelines/<source>/pipeline.yaml` на `src/bioetl/configs/pipelines/<source>.yaml`
   - Привести примеры конфигов в соответствие с фактическим расположением

3. **`DATA_SOURCES.md`** — обновить описание конфигурации:
   - Заменить `config_path: src/bioetl/configs/pipelines/<source>/pipeline.yaml` на фактический путь
   - Привести описание структуры источников в соответствие с `MODULE_RULES.md`

4. **`REFACTOR_PLAN.md`** — исправить ссылки на структуру:
- Обновить раздел "План по источникам" (строка 713): заменить ссылку на `src/bioetl/configs/pipelines/document.yaml`, где описана секция источника Crossref
