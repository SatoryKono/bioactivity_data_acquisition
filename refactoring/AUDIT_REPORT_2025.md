...
| P1-4 | Не все пайплайны имеют `__all__` | ❌ **НЕ ИСПРАВЛЕНО** | Проверка показала: только `pipelines/__init__.py` и `pipelines/document_enrichment.py` имеют `__all__`. Файлы `activity.py`, `assay.py`, `document.py`, `target.py`, `testitem.py` не имеют явного `__all__` |
...
|----|----------|-------------------|----------------|-----------|
| P0-1 | Структура `src/bioetl/sources/<source>/` | ❌ **НЕ ИСПРАВЛЕНО** | ❌ **НЕ ИСПРАВЛЕНО** | Без изменений (34 упоминания в 7 файлах) |
...
| P1-1 | `FallbackManager` не реализован | ⚠️ **ЧАСТИЧНО** | ⚠️ **ЧАСТИЧНО** | Без изменений (реализован, но не интегрирован) |
...
| P1-4 | Не все пайплайны имеют `__all__` | ❌ **НЕ ИСПРАВЛЕНО** | ❌ **НЕ ИСПРАВЛЕНО** | Без изменений (5 файлов без `__all__`) |
...
**Прогресс:** Без существенных изменений после обновления 2025-01-28

**Проверенные проблемы:**
- ✅ Все подтверждённые исправления остаются валидными (P0-2, P0-3, P0-4, P0-7, P2-1)
- ❌ Неисправленные проблемы остаются без изменений (P0-1, P0-6, P1-3, P1-4, P1-5, P2-2)
- ⚠️ Частично решённые проблемы остаются в том же состоянии (P0-5, P1-1, P1-2)
- 📊 Новых исправлений не обнаружено
- 📊 Новых проблем не выявлено

**Метрики прогресса:**
- **P0:** 4/7 полностью исправлено, 1 частично (~64%)
- **P1:** 0/5 полностью исправлено, 2 частично (~20%)
- **P2:** 1/2 исправлено (50%)
- **Общий прогресс:** ~40% (6 из 14 проблем исправлено полностью или частично)
...
- ❌ **НЕ ИСПРАВЛЕНО (3):** P1-3 (property-based тесты), P1-4 (публичный API __all__ — подтверждено: 5 файлов без __all__), P1-5 (тесты бит-идентичности)
...
| P1 | 5 | 0 полностью + 2 частично | ~20% (P1-4 подтверждено как проблема) |
| P2 | 2 | 1 исправлено (P2-1) | 50% |
| **Итого** | **14** | **6 полностью или частично** | **~40%** |
