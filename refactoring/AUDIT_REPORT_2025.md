...
#### Критические проблемы (P0) — **ЧАСТИЧНО ИСПРАВЛЕНО**
...
| P0-2 | Устаревшие ссылки `@test_refactoring_11` | ✅ **ЧАСТИЧНО ИСПРАВЛЕНО** | Большинство ссылок обновлено на `@test_refactoring_32` (498 упоминаний). **Осталось:** только в `AUDIT_REPORT_2025.md` (7 упоминаний в самом отчёте) и возможно в `IO_SCHEMAS_AND_DIAGRAMS.md` |
| P0-3 | Устаревшие ссылки `@Pipeline_Unification` | ⚠️ **ПОЧТИ ИСПРАВЛЕНО** | Осталось только в отчётах (`AUDIT_REPORT.md`, `AUDIT_REPORT_2025.md`) |
...
**Прогресс:** Частичный (~30% P0 проблем исправлено)

**Критические проблемы:**
- ✅ **ИСПРАВЛЕНО (2):** P0-2 (устаревшие ссылки — частично), P0-7 (CLI документация)
- ⚠️ **ЧАСТИЧНО (2):** P0-3 (Pipeline_Unification — только в отчётах), P0-5 (tests/sources — существует для адаптеров)
- ❌ **НЕ ИСПРАВЛЕНО (3):** P0-1 (структура sources), P0-4 (инвентаризация CSV), P0-6 (configs/sources)
...
- **Найдено:** 
  - Большинство ссылок обновлено на `@test_refactoring_32` (498 упоминаний)
  - `@test_refactoring_11` осталось только в `AUDIT_REPORT_2025.md` (7 упоминаний в самом отчёте) и возможно в `IO_SCHEMAS_AND_DIAGRAMS.md`
  - `@Pipeline_Unification` осталось только в отчётах
- **Действие:** Очистить упоминания из отчётов и проверить `IO_SCHEMAS_AND_DIAGRAMS.md`
...
|-----------|-----------------|------------|-----------|
| P0 | 7 | 2 полностью + 2 частично | ~43% |
| P1 | 5 | 0 (частично CLI уже был) | ~10% |
| P2 | 2 | 0 | 0% |
| **Итого** | **14** | **~3-4** | **~25-30%** |
