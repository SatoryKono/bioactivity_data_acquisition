...
### Критические проблемы (P0) — **ИСПРАВЛЕНО** (6/7 полностью исправлено)

| ID | Проблема | Актуальный статус | Детали |
|----|----------|-------------------|--------|
| P0-1 | Структура `src/bioetl/sources/<source>/` | ⚠️ **ЧАСТИЧНО** | Структура `sources/` существует и правильная для внешних источников (crossref, pubmed, openalex, semantic_scholar, iuphar, uniprot). ChEMBL пайплайны дублируются между `pipelines/` (монолитные файлы) и `sources/chembl/` (прокси) |
...
5. ❌ Добавить property-based тесты (P1-3) — опционально
6. ❌ Добавить автоматическую проверку бит-идентичности (P1-5) — опционально
