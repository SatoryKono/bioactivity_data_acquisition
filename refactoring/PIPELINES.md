...
> **Обновление:** Структура `src/bioetl/sources/` остаётся канонической для внешних источников данных. Модульные реализации ChEMBL находятся в `src/bioetl/sources/chembl/<entity>/`, а файлы `src/bioetl/pipelines/*.py` сохранены как совместимые прокси, которые реэкспортируют новые пайплайны. Базовый класс `DocumentPipeline` живёт в `bioetl.sources.chembl.document.pipeline`, тогда как модуль `bioetl.sources.document.pipeline` публикует только `AdapterDefinition` и `ExternalEnrichmentResult` для адаптеров. Для внешних обогащений каждый адаптер публикует `*_ADAPTER_DEFINITION` (см. `bioetl/sources/{crossref,pubmed,openalex,semantic_scholar}/pipeline.py`), который регистрируется потребителем `bioetl.sources.chembl.document.pipeline.DocumentPipeline`. Совместимость старого API поддерживается через прокси `src/bioetl/pipelines/document.py` (`bioetl.pipelines.document.DocumentPipeline`).
