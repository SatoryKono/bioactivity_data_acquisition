...
from scripts import PIPELINE_REGISTRY, create_pipeline_app
from tests.e2e.helpers import snapshot_artifacts, verify_bit_identical_outputs
...
def _build_cli_command(pipeline_cls: type[PipelineBase]) -> tuple[click.Command, PipelineCommandConfig]:
    ...
    registry_entry = PIPELINE_REGISTRY["chembl_assay"]
    config = replace(
        registry_entry,
