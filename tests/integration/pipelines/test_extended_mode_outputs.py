...
def test_pipeline_run_emits_extended_artifacts(tmp_path: Path) -> None:
    ...
    assert "manifest" in qc_artifacts
    ...
    assert manifest.get("git_commit") is None
    assert manifest["inputs"]["sources"] == ["chembl"]
    assert "generated_at" in manifest
    outputs = manifest["outputs"]
    assert outputs["dataset"].endswith("integration.csv")
    assert Path(qc_artifacts["manifest"]) == manifest_path
    assert outputs["metadata"].endswith("integration_meta.yaml")
