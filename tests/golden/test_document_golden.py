"""Golden regression tests for ChemblDocumentPipeline."""
...
import pytest
from tests.support.factories import load_sample_document_dataframe
...
def test_document_pipeline_golden_snapshot(
    pipeline_config_fixture,
) -> None:
    """ChemblDocumentPipeline output must match committed golden artefacts."""

    pipeline_config_fixture.validation.schema_out = "bioetl.schemas.chembl_document_schema.DocumentSchema"  # type: ignore[attr-defined]
    ...
    pipeline_config_fixture.determinism.hashing.business_key_fields = ("document_chembl_id",)  # type: ignore[attr-defined]

    golden_run_id = "golden-document-v1"
    pipeline = ChemblDocumentPipeline(config=pipeline_config_fixture, run_id=golden_run_id)  # type: ignore[arg-type]
    frame = load_sample_document_dataframe()
