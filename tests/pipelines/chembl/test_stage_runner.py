...
class DummyPipeline(UnifiedPipelineBase):
    ...
    def extract(self, descriptor: str, options: StageExecutionOptions) -> pd.DataFrame:
    ...
    def save_results(self, df: pd.DataFrame, artifacts: WriteArtifacts, options: StageExecutionOptions) -> WriteResult:
...
def test_run_stage_respects_transform_context(tmp_path: Path) -> None:
    ...
    transformed = run_chembl_stage(pipeline, "transform", df=df, output_dir=tmp_path)
